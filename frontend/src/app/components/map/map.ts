...
import { BienImmobilier, PropertyService } from '../../services/biens';
...
export class MapComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.propertyService.getProperties().subscribe((data: BienImmobilier[]) => {
  ...
  biens.forEach((bien) => {
    ...
    const lat = bien.latitude;
    const lng = bien.longitude;
    ...
    icon: L.divIcon({
      ...
      html: `
        ...
        <div class="custom-marker">
          ...
          <div class="label">${bien.designation}</div>
        </div>
      `,
      iconSize: [28, 38],
      iconAnchor: [14, 38],
...
onListClicked() {
  ...
  if (this.listVisible()) {
    const entites: string[] = (this.biens as BienImmobilier[])
      .map((b: BienImmobilier) => b.entiteGestionnaire)
...
filtrerParEntite(entite: string): void {
  ...
  this.markers.forEach(({ bien, marker }) => {
    const doitAfficher = bien.entiteGestionnaire === entite;
    if (doitAfficher) {
      marker.addTo(this.map);
    } else {
      this.map.removeLayer(marker);
  ...
  if (bienActuel && bienActuel.entiteGestionnaire !== entite) {
    this.selectedProperty.set(null);
    this.sidebarVisible.set(false);
  }
}
